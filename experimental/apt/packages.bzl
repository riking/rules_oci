...
def _clear(str):
    return  str.replace("~", "_t_").replace("+", "_p_").replace(":", "_c_")
...
_DEB_BIN_TMPL='''\
    run_binary(
        name = "{dist}_{arch}_{name}_{version}_help",
        tool = "@{fsroot}//:binary",
        outs = ["{dist}_{arch}_{name}_{version}_out"],
        args = [
            "--help"
        ]
    )
...
def _debian_package_index_impl(rctx):
    ...
    package_defs=[
        ...
        "",
        """
def _deb_fsroot_impl(rctx):
    for deb in rctx.attr.debs:
        rctx.extract(
            rctx.path(deb),
            output = "fsroot"
        )
...
LD_LIBRARY_PATH=$wksp/fsroot/usr/lib/aarch64-linux-gnu exec $wksp/fsroot/usr/bin/bsdtar $@
''')

    rctx.file("BUILD.bazel", '''
exports_files(["fsroot"])
...
deb_fsroot = repository_rule(
    implementation = _deb_fsroot_impl,
    attrs = {
    ...
    for (dist, architecture, name, version, closure) in closures:
        fsroot = "{dist}_{arch}_{name}_{version}_fsroot".format(
            dist = dist,
            arch = architecture,
            name = _clear(name),
